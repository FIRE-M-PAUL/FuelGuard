...
def get_analytics_bundle() -> dict[str, Any]:
    ...
    expenses_today = db.execute(
        "SELECT COALESCE(SUM(amount), 0) AS s FROM expenses WHERE date(date) = date(?)",
        (today,),
    ).fetchone()

    revenue_today = float(sales_today["rev"] or 0)
    exp_today = float(expenses_today["s"] or 0)
    ...
    exp_m = float(monthly_exp["s"] or 0)
    ...
    monthly_series = db.execute(
        ...
        FROM fuel_sales
        WHERE date(sale_date) >= date('now', '-11 months')
        GROUP BY ym
        ORDER BY ym ASC
        """
    ).fetchall()
    ...
    daily_sales_14 = db.execute(
        ...
        FROM fuel_sales
        WHERE date(sale_date) >= date('now', '-13 day')
    ...
    return {
        ...
        "net_after_purchases_today": revenue_today - exp_today - purchase_cost_today,
        "fuel_purchase_cost_month": purchase_cost_m,
        "net_after_purchases_month": rev_m - exp_m - purchase_cost_m,
        "fuel_purchase_cost_year": purchase_cost_y,
        "net_after_purchases_year": rev_y - exp_y - purchase_cost_y,
